...
// Replace the injected fetch script to not slice and call initPagination
// The script I injected starts with "document.addEventListener('DOMContentLoaded', function() {"
// Let's replace the whole script tag.

const scriptStart = "<script>\n      document.addEventListener('DOMContentLoaded', function() {\n        const sliderList = document.querySelector('.slider .list');";
const scriptRegex = /<script>\s*document\.addEventListener\('DOMContentLoaded',\s*function\(\)\s*\{\s*const sliderList = document\.querySelector\('\.slider \.list'\);[\s\S]*?\} \/\/\s*Actually wait[\s\S]*?<\/script>|<script>\s*document\.addEventListener\('DOMContentLoaded',\s*function\(\)\s*\{\s*const sliderList = document\.querySelector\('\.slider \.list'\);[\s\S]*?\}\);[\s\S]*?<\/script>/;
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch('cms-books.json')
          ...
          .then(books => {
            ...
            // Re-initialize pagination after dynamic injection
...
if (indexContent.match(scriptRegex)) {
  indexContent = indexContent.replace(scriptRegex, newFetchScript);
  fs.writeFileSync(indexPath, indexContent, 'utf8');
  console.log('Successfully replaced fetch script in index.html');
} else {
  console.log('Fetch script not found in index.html. Check the regex.');
}
